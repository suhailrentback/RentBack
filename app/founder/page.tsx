...
export default function FounderPage() {
  ...
  const L =
    lang === "ur"
      ? {
          ...
          p1:
            "ہمارے بانی سوہیل احمد کی قیادت میں رینٹ بیک پاکستان اور اس سے آگے کرایہ ادائیگیوں کا مستقبل بنا رہا ہے۔ ہمارا مشن روزمرہ خرچ کو مالی ترقی، تحفظ اور انعامات کے موقع میں بدلنا ہے۔",
          p2:
            "یہ ڈیمو پہلا قدم ہے — یہ دکھاتا ہے کہ ٹیکنالوجی، ڈیزائن اور اعتماد کیسے اکٹھے ہو سکتے ہیں۔ جلد ہی رینٹ بیک ایک مکمل پلیٹ فارم بنے گا جو کرایہ داروں، مکان مالکان اور شراکت داروں سب کے لیے یکساں فائدہ مند ہوگا۔",
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 dark:bg-[#0b0b0b] dark:text-white">
      {/* Simple top bar (no bottom nav on this page) */}
      ...
      <main className="mx-auto max-w-3xl px-4 py-8">
        ...
        <section className="mt-4 rounded-2xl border border-black/10 dark:border-white/10 p-5 bg-white/50 dark:bg-white/5 backdrop-blur space-y-3 leading-7">
          ...
          {/* Action buttons under the paragraph block */}
          <div className="mt-4 flex gap-2">
            <button
              type="button"
              onClick={() => r.push("/")}
              className="h-10 px-3 rounded-xl border border-black/10 dark:border-white/10 hover:bg-black/5 dark:hover:bg-white/10 text-sm"
            >
              {L.home}
            </button>
            <button
              type="button"
              onClick={() => r.push("/sign-in")}
              className="h-10 px-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm"
            >
              {L.signIn}
            </button>
          </div>
