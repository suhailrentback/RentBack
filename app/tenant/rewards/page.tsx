...
import { getRewardsContext, redeemVoucher, formatPKR, ensureSeed } from "@/lib/demo";
...
export default function RewardsPage() {
  ...
  function redeem(partner:string, cost:number){
    try { redeemVoucher(partner, cost); refresh(); alert("Redeemed in demo!"); }
      ...
      <div className="max-w-md mx-auto px-4 py-4">
        ...
        <div className="grid gap-2">
          {[["Foodpanda PK",500],["Careem Credits",800],["Daraz Voucher",1000]].map(([name, cost])=>(
            <button key={name as string} onClick={()=>redeem(name as string, cost as number)}
