...
function toCSV(rows: DiscRow[]) {
  ...
  a.href = url;
  a.download = `admin_discrepancies_${Date.now()}.csv`;
  a.click();
...
export default function AdminDiscrepanciesPage() {
  ...
  useEffect(() => {
    setRows([
      { id: "d_101", receiptId: "p_abc123", reason: "Amount mismatch", amount: 1500 },
      { id: "d_102", receiptId: "p_xyz789", reason: "Duplicate payment", amount: 65000 },
    ]);
  }, []);
    ...
    <AppShell role="admin" title="Discrepancy Report">
      <div className="p-4 space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-lg font-semibold">Discrepancy Report</h1>
          <button
            onClick={() => rows && toCSV(rows)}
