...
const items = [
  { href: "/tenant", labelKey: "bottom.home", labelFallback: "Home", icon: HomeIcon },
  { href: "/tenant/pay", labelKey: "bottom.pay", labelFallback: "Pay", icon: PayIcon },
  { href: "/tenant/rewards", labelKey: "bottom.rewards", labelFallback: "Rewards", icon: RewardsIcon },
  { href: "/tenant/profile", labelKey: "bottom.profile", labelFallback: "Profile", icon: ProfileIcon },
];

export default function RoleNavTenant({ currentPath }: Props) {
  return (
    <nav className="h-14 flex items-stretch justify-between">
      {items.map(({ href, labelFallback, icon: Icon }) => {
